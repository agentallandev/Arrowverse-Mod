...
public class MCreatorSpeedDown extends Elementsarrowverse.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public MCreatorSpeedDown(Elementsarrowverse instance) {
		super(instance, 57);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.speeddown", GLFW.GLFW_KEY_C, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (!(Minecraft.getInstance().currentScreen instanceof ChatScreen)) {
			if (event.getKey() == keys.getKey().getKeyCode() && event.getAction() == GLFW.GLFW_PRESS) {
				arrowverse.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(PlayerEntity entity) {
			...
			return;
		{
			...
			MCreatorLessSpeed.executeProcedure($_dependencies);
		}
	}
}
