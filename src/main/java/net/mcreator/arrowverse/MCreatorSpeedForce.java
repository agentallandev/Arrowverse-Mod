...
public class MCreatorSpeedForce extends Elementsarrowverse.ModElement {
	@ObjectHolder("arrowverse:speedforce")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("speedforce"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("arrowverse:speedforce")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("arrowverse:speedforce"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("arrowverse:speedforce"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("arrowverse:speedforcebiome")),};
	}
		...
		public BiomeProviderCustom(World world) {
			...
			for (Biome biome : this.biomes) {
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(MCreatorSpeedForceBlock.block.getDefaultState().getBlock(), biome.getSurfaceBuilder()
								.getConfig().getTop().getBlock(), biome.getSurfaceBuilder().getConfig().getUnder().getBlock());
					}
